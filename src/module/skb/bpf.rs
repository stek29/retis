...
pub(super) fn unmarshal_arp(raw_section: &BpfRawSection) -> Result<SkbArpEvent> {
    ...
    Ok(SkbArpEvent {
        ...
        spa: helpers::parse_ipv4_addr(u32::from_be(raw.spa))?,
        tha: helpers::parse_eth_addr(&raw.tha)?,
        tpa: helpers::parse_ipv4_addr(u32::from_be(raw.tpa))?,
    })
}
...
pub(super) fn unmarshal_ipv4(raw_section: &BpfRawSection) -> Result<SkbIpEvent> {
    ...
    Ok(SkbIpEvent {
        saddr: helpers::parse_ipv4_addr(u32::from_be(raw.src))?,
        daddr: helpers::parse_ipv4_addr(u32::from_be(raw.dst))?,
