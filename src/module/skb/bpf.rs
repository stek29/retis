...
pub(super) fn unmarshal_packet(event: &mut SkbEvent, raw_section: &BpfRawSection) -> Result<()> {
    ...
    let eth = match EthernetPacket::new(&raw.packet[..(raw.capture_len as usize)]) {
        Some(eth) => eth,
        None => return Ok(()),
    };
    ...
    match eth.get_ethertype() {
        ...
        EtherTypes::Ipv4 => {
            if let Some(ip) = Ipv4Packet::new(eth.payload()) {
                ...
                unmarshal_l3(event, ip.get_next_level_protocol(), ip.payload())?;
            };
        }
        EtherTypes::Ipv6 => {
            if let Some(ip) = Ipv6Packet::new(eth.payload()) {
                event.ip = Some(unmarshal_ipv6(&ip)?);
                unmarshal_l3(event, ip.get_next_header(), ip.payload())?;
            };
        }
        _ => (),
    }
